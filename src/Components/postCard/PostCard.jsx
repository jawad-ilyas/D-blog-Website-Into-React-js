...
function PostCard({ bgColor, timeDuration, blogTitle, blogParagrahp, categoryColor }) {
    return (
        <div className={`bg-${bgColor}-500 ps-4 `}>
            <div className=' p-4 h-96 flex-1 flex flex-col justify-between items-center bg-white  shadow-lg'>
                ...
                <div className='space-y-4'>
                    <h1 className='text-wrap text-EeieBlack hover:underline text-4xl font-semibold cursor-pointer'>
                        {blogTitle}
                    </h1>
                    <p className='text-Gray text-base'>{blogParagrahp}</p>
                </div>
